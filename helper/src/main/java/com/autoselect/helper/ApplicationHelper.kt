...
object ApplicationHelper : LoggerHelper {
    ...
    fun registerAppStatusChangedListener(any: Any, listener: AHelper.OnAppStatusChangedListener) =
    ...
    fun installAppSilent(
        ...
        isRooted: Boolean = !isAppSystem || RootHelper.isRooted
    ): Boolean = installAppSilent(getFileByPath(filePath), params, isRooted)

    @JvmOverloads
    fun installAppSilent(
        file: File?, params: String? = null, isRooted: Boolean = !isAppSystem || RootHelper.isRooted
    ): Boolean = file?.run {
        when {
