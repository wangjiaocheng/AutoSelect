...
import com.autoselect.helper.ConvertHelper.DAY
import com.autoselect.helper.ConvertHelper.date2Millis
import com.autoselect.helper.ConvertHelper.millis2Date
import com.autoselect.helper.ConvertHelper.millis2String
import com.autoselect.helper.ConvertHelper.millis2TimeSpan
import com.autoselect.helper.ConvertHelper.millis2TimeSpanFit
import com.autoselect.helper.ConvertHelper.millis2TimeSpanFitByNow
...
object DateHelper : LoggerHelper {
    ...
    fun getNowOffsetDay(date: Date?, @ConvertHelper.TimeUnit unit: Int): String =
    ...
    fun getStringByTimeSpan(
        timeSpan: Long, @ConvertHelper.TimeUnit unit: Int, time: String? = nowString
    ): String? = time?.let { millis2String(string2Millis(time) + timeSpan2Millis(timeSpan, unit)) }

    fun getStringByTimeSpan(
        timeSpan: Long, @ConvertHelper.TimeUnit unit: Int, millis: Long? = nowMillis
    ): String? = millis?.let { millis2String(millis + timeSpan2Millis(timeSpan, unit)) }

    fun getStringByTimeSpan(
        timeSpan: Long, @ConvertHelper.TimeUnit unit: Int, date: Date? = nowDate
    ...
    @JvmOverloads
    fun getTimeSpan(
        time1: String?, @ConvertHelper.TimeUnit unit: Int, time2: String? = nowString
    ): Long = millis2TimeSpan(string2Millis(time2) - string2Millis(time1), unit)

    @JvmOverloads
    fun getTimeSpan(
        millis1: Long?, @ConvertHelper.TimeUnit unit: Int, millis2: Long? = nowMillis
    ): Long = millis2TimeSpan((millis2?.let { it } ?: 0L) - (millis1?.let { it } ?: 0L), unit)

    @JvmOverloads
    fun getTimeSpan(date1: Date?, @ConvertHelper.TimeUnit unit: Int, date2: Date? = nowDate): Long =
