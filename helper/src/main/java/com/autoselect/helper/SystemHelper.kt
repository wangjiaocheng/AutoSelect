...
import com.autoselect.helper.ActivityHelper.launcherActivityName
...
object SystemHelper {
    ...
    val buildSerial: String
        ...
        get() = if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) Build.SERIAL else Build.getSerial()
        ...
        private fun updateLanguage(context: Context, locale: Locale) = context.resources.run {
            configuration.locale.let {
                if (!equals(it.language, locale.language) || !equals(it.country, locale.country)) {
                    when {
                        VersionHelper.aboveJellyBeanMR1 -> {
