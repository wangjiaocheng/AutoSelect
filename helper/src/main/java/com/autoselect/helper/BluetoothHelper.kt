...
object BluetoothHelper {
    ...
    private val registerBTReceiver = {
        ...
        }.let { AHelper.app.registerReceiver(mReceiver, it) }
        mNeed2unRegister = true
    }//注册蓝牙广播接收器
    val stopSearch = bluetoothAdapter?.run {
    ...
    fun createBind(btDevice: BluetoothDevice, pingCode: String): Boolean =
        btDevice.apply { setPin(pingCode.toByteArray()) }.createBond()//蓝牙匹配方法
    ...
    val release = {
        ...
        if (mNeed2unRegister) {
            AHelper.app.unregisterReceiver(mReceiver)
