...
import kotlinx.android.synthetic.main.activity_cloud_preview.*
...
class MapCloudPreviewActivity : AppCompatActivity() {
    ...
    private val pageChangeListener: ViewPager.OnPageChangeListener =
        object : ViewPager.OnPageChangeListener {
            ...
            override fun onPageSelected(position: Int) {
                ...
                pageIndex = position
                title_des_text.text = "${(pageIndex + 1)}/$pageCount"
            }
        }
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        title_des_text.paint.isFakeBoldText = true
        viewpager_photo.adapter = listViews?.let { MapPagerAdapter(it) }.apply { adapter = this }
        viewpager_photo.currentItem = intent.getIntExtra("position", 0).apply { pageIndex = this }
        viewpager_photo.setOnPageChangeListener(pageChangeListener)
        ...
        if (pageCount > 0) title_des_text.text = "${(pageIndex + 1)}/$pageCount"
