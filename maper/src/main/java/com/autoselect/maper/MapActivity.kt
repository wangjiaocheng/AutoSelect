...
class MapActivity : AppCompatActivity(), AnkoLogger, AMap.OnMapScreenShotListener,
    ...
    internal inner class BusLineDialog(context: Context, theme: Int) :
        ...
        override fun onCreate(savedInstanceState: Bundle) {
            ...
            if (currentPage <= 0) preButton?.isEnabled = false
            if (currentPage >= (busLineResult?.pageCount ?: 0) - 1) nextButton?.isEnabled = false
            listView?.apply {
        ...
        override fun onClick(view: View) {
            dismiss()
            if ((view == preButton)) currentPage-- else if ((view == nextButton)) currentPage++
            showProgressDialog
