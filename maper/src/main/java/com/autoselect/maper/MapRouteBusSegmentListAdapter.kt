...
import kotlinx.android.synthetic.main.item_bus_segment_ex.view.*
...
class MapRouteBusSegmentListAdapter(
    ...
    override fun getView(position: Int, convertView: View?, viewGroup: ViewGroup): View? =
        (convertView ?: View.inflate(context, R.layout.item_bus_segment, null).apply {
            tag = ViewHolder().apply {
                busDirIcon = bus_dir_icon
                busLineName = bus_line_name
                busStationNum = bus_station_num
                busExpandImage = bus_expand_image
                busDirUp = bus_dir_icon_up
                busDirDown = bus_dir_icon_down
    ...
    constructor(private val viewHolder: ViewHolder?, private var schemeBusStep: SchemeBusStep) :
        ...
        private fun addBusStation(station: BusStationItem) =
            (View.inflate(context, R.layout.item_bus_segment_ex, null)).apply {
                bus_line_station_name_ex.apply { text = station.busStationName }
            }.let { viewHolder?.expandContent?.addView(it) }

        private fun addRailwayStation(station: RailwayStationItem) =
            (View.inflate(context, R.layout.item_bus_segment_ex, null)).apply {
                bus_line_station_name_ex
